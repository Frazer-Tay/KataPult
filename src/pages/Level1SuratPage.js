...
import { suratData } from '../data/level1Practice';
import { recordLearnerActivity } from '../utils/activityTracker';
import { useSettings } from '../contexts/SettingsContext';
import styles from './Level1Practice.module.css';

const Level1SuratPage = () => {
  const { englishAssist } = useSettings();
  const [selectedTask, setSelectedTask] = useState(`surat${suratData[0].id}`);
  const [text, setText] = useState('');
  const [wordCount, setWordCount] = useState(0);
  const [isReviewing, setIsReviewing] = useState(false);
  const [checklist, setChecklist] = useState({});

  const currentPrompt = suratData.find(s => `surat${s.id}` === selectedTask) || suratData[0];
  ...
  const toggleReview = () => {
    ...
    if (nextReviewState) {
      ...
      }).catch((error) => console.warn('Failed to record surat review:', error));
    }
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <h1>{englishAssist ? 'Part V: Writing Personal Letters' : 'Bagian V: Menulis Surat Pribadi'}</h1>
        <p>{englishAssist ? 'Choose a letter writing task from the list below, then type your letter.' : 'Pilih tugas menulis surat dari daftar di bawah, lalu ketik surat Anda.'}</p>
      </header>
      ...
      <div className={styles.centeredCard} style={{ maxWidth: '900px' }}>
        ...
        <div style={{ marginBottom: '2rem', display: 'flex', gap: '1rem', flexWrap: 'wrap' }}>
          {suratData.map(surat => (
            <button
              ...
              style={{
                opacity: selectedTask === `surat${surat.id}` ? 1 : 0.6,
                backgroundColor: surat.modelEssay ? '#34495e' : '#2c3e50',
                border: surat.modelEssay ? '2px solid #f39c12' : 'none'
              }}
              onClick={() => setSelectedTask(`surat${surat.id}`)}
              title={surat.modelEssay ? (englishAssist ? "Model Answer Available" : "Model Jawaban Tersedia") : ""}
            >
              {surat.title} {surat.modelEssay && "⭐"}
            </button>
          ))}
        </div>
        ...
        <div className={styles.writingToolbar}>
          <span className={styles.progressIndicator}>
            {englishAssist ? 'Target: ~150-200 Words' : 'Target: ~150-200 Kata'}
          </span>
          <span className={`${styles.wordCounter} ${wordCount >= 150 ? styles.targetReached : ''}`}>
        ...
        <textarea
          ...
          placeholder={englishAssist ? "Start typing your letter here..." : "Mulai mengetik surat Anda di sini..."}
          value={text}
        ...
        {isReviewing && (
          <div className={styles.checklist}>
            ...
            <p style={{ marginBottom: '1rem', color: '#64748b' }}>{englishAssist ? 'Honestly check the criteria below to evaluate your letter.' : 'Centang kriteria di bawah ini secara jujur untuk mengevaluasi surat Anda.'}</p>
            ...
            {currentPrompt.modelEssay && (
              <div className={styles.explanationBox} style={{ marginTop: '2rem', backgroundColor: '#f8fafc', border: '1px solid #cbd5e1', textAlign: 'left' }}>
                <h3 style={{ marginBottom: '1rem', color: '#0f172a' }}>{englishAssist ? 'Model Letter Example:' : 'Contoh Model Surat:'}</h3>
...
export default Level1SuratPage;
