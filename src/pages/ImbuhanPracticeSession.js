...
const ImbuhanPracticeSession = ({ practiceSet, onBack, sessionTitle }) => {
  ...
  useEffect(() => {
    const initPage = async () => {
      setIsLoading(true); setError(null);
      try {
        const filteredData = practiceSet || imbuhanData.filter(item => item.root && item.targetWord && item.sentence && item.hint);
  ...
  useEffect(() => {
    ...
    if (isCompletedNow) {
        if (localStorage.getItem(storageKey)) { // Clear on actual completion only
            localStorage.removeItem(storageKey);
        }
        return;
    }
  ...
  if (isCompleted) {
    const completionText = isReviewingMistakes
        ? (englishAssist ? "✨ Imbuhan Review Session Complete! ✨" : "✨ Sesi Review Imbuhan Selesai! ✨")
        : (englishAssist ? "✨ Imbuhan Practice Complete! ✨" : "✨ Latihan Imbuhan Selesai! ✨");
      ...
      <div className={styles.container}>
        ...
        {!isReviewingMistakes && mistakesToShow > 0 && (
          <p className="completionSubMessage">
            {englishAssist ? `You had ${mistakesToShow} incorrect items in the initial round.` : `Anda memiliki ${mistakesToShow} item yang salah pada putaran awal.`}
          </p>
        )}
        <div className={styles.completionActions}>
          ...
          <button className="primaryButton" onClick={handleReshuffleAll}>
            {isReviewingMistakes
                ? (englishAssist ? 'Restart All' : 'Mulai Lagi Semua')
                : (englishAssist ? 'Restart All' : 'Ulangi Semua')}
          </button>
          {onBack && (
            <button className="secondaryButton" onClick={onBack} style={{marginTop: '10px'}}>
  ...
  return (
    <div className={styles.container} ref={pageRef} tabIndex={-1}>
      <ProgressBar current={currentIndex + 1} total={totalItemsInSet} label={sessionTitle ? sessionTitle : (isReviewingMistakes ? "Review Kesalahan Imbuhan" : "Imbuhan")} />
      {onBack && (
      ...
      <div className={styles.card}>
        <p className={styles.label}>{englishAssist ? 'Complete the following sentence:' : 'Lengkapi kalimat berikut:'}</p>
      ...
      <PracticeActions>
        ...
        {!isAnswered && (
          <button className="primaryButton" onClick={checkAnswer} disabled={!userInput.trim()}>
            {englishAssist ? 'Check Answer' : 'Periksa'}
          </button>
        )}
...
export default ImbuhanPracticeSession;
